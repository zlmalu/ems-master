		...
		<div id="page-wrapper">
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        ...
                        <div class="panel-body">
                            <table width="100%" class="table table-striped table-bordered table-hover" id="emsClassRoomTable">
                                <thead>
                                    <tr>
                                        <th>id</th>
                                                                                <th>roomName</th>
            ...
            $(document).ready(function() {
                ...
                var table = $('#' + tableId).DataTable({
                    ...
                    "rowId" : 'id',// 设置主键字段名称
                    "ajax": {
                        "url": "${pageContext.request.contextPath}/ems/classRoom/list", // 请求列表数据的url
                        "type": "GET" // http方法
                    },
                ...
                $('#' + tableId + ' tbody').on( 'click', 'a', function () {
                	// this表示a标签对应dom，$(this)将其转为jQuery对象，获取按钮所在行的json对象:ClassRoomDto
                    var data = table.row( $(this).parents('tr') ).data();
                	// 调用修改方法
                	if ($(this).attr('name') == 'update') {
                		updateEmsClassRoom(data.id);
                	}
                	// 调用删除方法
					if ($(this).attr('name') == 'delete') {
						deleteEmsClassRoom (data.id);
                	}
                } );
             	// 设置列表多选
            ...
            function deleteEmsClassRoom (id) {
            	...
            	$.post ('${pageContext.request.contextPath}/ems/classRoom/delete', {id : id}, function (result) {
		    		if (result.success) {
		    			var tableId = 'emsClassRoomTable';
				    	var table = $('#' + tableId).DataTable();
				    	// 删除选中的行
            ...
            function deleteEmsClassRooms () {
		    	...
		    	var tableId = 'emsClassRoomTable';
		    	var table = $('#' + tableId).DataTable();
		    	// 查找样式为info的行，包含所有ClassRoomDto属性
		    	...
		    	// 发送ajax请求，如果成功刷新本页面，否则提示教室操作失败
		    	deleteEmsClassRoom(ids);
		    }
		    </script>
		</div>