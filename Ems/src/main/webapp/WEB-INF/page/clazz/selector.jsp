	...
	pageEncoding="UTF-8"%>
		...
		<div class="modal-content">
			<div class="modal-header">
		        ...
		        <h4 class="modal-title" id="emsClazzSelectorModalLabel">选择班级</h4>
		    </div>
            <!-- /.row -->
            ...
            $(document).ready(function() {
            	...
            	var tableId = 'emsClazzSelectorTable';
            	var cols = [];
            	cols.push ({ "data": "id" , "orderable" : false});
            	            	cols.push ({ "data": "clazzName" , "orderable" : false});
            	            	            	cols.push ({ "data": "majorName" , "orderable" : false});
            	            	var orderIndex = 0;
                ...
                var table = $('#' + tableId).DataTable({
                    ...
                    "rowId" : 'id',// 设置列为id
                    "ajax": {
                        "url": "${pageContext.request.contextPath}/ems/clazz/selector/list" + (opts.params ? '?params=' + opts.params : ''), // 请求列表数据的url
                        "type": "GET"
                    },
                ...
                $('#' + tableId + '_length').append (" <a class='btn btn-primary btn-sm' onclick='selectEmsClazzOK();'>确定</a> <a class='btn btn-primary btn-warning btn-sm' onclick='closeEmsClazzSelector();'>取消</a>");
                
                // 确定按钮点击事件
                window.selectEmsClazzOK = function () {
             	...
             	// 确定按钮点击事件
                window.closeEmsClazzSelector = function () {
                	$('#' + tableId).parents ('.modal').modal('hide');
                }
            });
